...
class _ExpenseFormPageState extends ConsumerState<ExpenseFormPage> {
  ...
  Future<void> _submit() async {
    ...
    if (_isEditing) {
      await ref.read(expenseFormProvider.notifier).update(expense);
    } else {
      await ref.read(expenseFormProvider.notifier).create(expense);
    }
    ...
    if (state is AsyncData) {
      AppSnackBar.success(context, _isEditing ? 'ব্যয় আপডেট হয়েছে' : 'নতুন ব্যয় যোগ হয়েছে');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_isEditing ? 'ব্যয় সম্পাদনা' : 'নতুন ব্যয়', style: AppTypography.subtitle1),
        centerTitle: true,
      ),
      body: Form(
        key: _formKey,
  ...
  Widget _buildCategoryDropdown(List<ExpenseCategory> categories) {
  ...
  Widget _buildStatusDropdown() {
    return DropdownButtonFormField<String>(
      ...
      items: const [
        DropdownMenuItem(value: 'completed', child: Text('পরিশোধিত')),
